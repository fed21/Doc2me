...
/* WEBPACK VAR INJECTION */(function($) {function isValidDate(dateString) {
  var ExDate = /^\d{4}-\d{2}-\d{2}$/;
  if (!dateString.match(ExDate)) return false; // Invalid format
...
$(document).ready(function () {
  var show = 1;
  var show = 0;
  var date = $("#reservationDate").value;
  $('#disp').click(function () {
    if (!$("#reservationDate").val() || isValidDate(date)) {
      alert("seleziona una data primadi continuare");
    } else if (show == 0) {
      $('#disp-panel').show();
      show = 1;
      console.log(show);
...
/******/ });
//# sourceMappingURL=disp-5f8e8a90eb5d186de96a.js.map